...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhkbXJoeHJ6dHh5cXBmemZ6eHhkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjEyNzg3MTEsImV4cCI6MjA3Njg1NDcxMX0.HzciPuYyYLTjLG-Uu65tCnnGpPTZlpem3UiHD71CMzs'; // Ganti dengan anon public key Anda
// =======================================================
...
try {
  if (typeof supabase === 'undefined' || !supabase.createClient) {
    throw new Error('Supabase library not loaded correctly. Check the script tag in HTML.');
  }
...
function placeholderImage(width = 400, height = 225, text = 'Image') {
  return `https://placehold.co/${width}x${height}/e2e8f0/cccccc?text=${encodeURIComponent(text)}`;
}
...
function showNotification(type, title, message) {
  ...
  if (type === 'success') {
    ...
    modalIconBg.className = 'mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-cyan-900/50 sm:mx-0 sm:h-12 sm:w-12 border border-cyan-500/30';
    closeBtn.className = 'inline-flex w-full justify-center rounded-full bg-cyan-500 px-6 py-2 text-sm font-semibold text-gray-900 shadow-sm hover:bg-cyan-400 sm:ml-3 sm:w-auto transition-colors duration-300';
    closeBtn.textContent = 'Excellent!';
  } else {
    // Style Error (Red Theme)
    modalIcon.className = 'ri-error-warning-line text-3xl text-red-400';
    modalIconBg.className = 'mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-900/50 sm:mx-0 sm:h-12 sm:w-12 border border-red-500/30';
    closeBtn.className = 'inline-flex w-full justify-center rounded-full bg-red-500 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-600 sm:ml-3 sm:w-auto transition-colors duration-300';
...
async function fetchHotelProfile() {
  ...
  const { data, error } = await supabaseClient.from('hotelProfile').select('*').single();
  if (error) {
...
async function fetchFacilities() {
  ...
  const { data, error } = await supabaseClient.from('hotelFacilities').select('*');
  if (error) {
...
async function loadAllData() {
  ...
  try {
    const [profileData, roomsData, facilitiesData, promotionsData, testimonialsData] = await Promise.all([fetchHotelProfile(), fetchRoomTypes(), fetchFacilities(), fetchPromotions(), fetchTestimonials()]);
    ...
    if (splash) splash.innerHTML = `<p class="text-red-500">Error loading data. Please refresh.</p>`;
  } finally {
...
function renderDataToHTML() {
  ...
  if (hotelProfile) {
    ...
    document.querySelectorAll('.hotel-name').forEach((el) => (el.textContent = hotelProfile.name || 'Hotelux'));
    ...
    if (heroImgEl) {
      ...
      if (rawUrl && typeof rawUrl === 'string') {
        ...
        if (cleanUrl) {
          ...
          heroImgEl.alt = hotelProfile.tagline || hotelProfile.name || 'Hero Image'; // Contoh: pakai tagline
          console.log('Set heroImgEl.src to:', heroImgEl.src);

          heroImgEl.onerror = function () {
            console.error('Error loading hero image from src:', heroImgEl.src);
            this.onerror = null; // Mencegah loop error jika placeholder juga gagal
            this.src = placeholderImage(1920, 1080, 'Image Load Error');
          };
        } else {
          console.warn('Cleaned Hero URL is empty. Setting placeholder.');
          heroImgEl.src = placeholderImage(1920, 1080, 'Invalid URL Data');
          heroImgEl.alt = 'Invalid image URL provided';
        }
      } else {
        console.warn("Hero image URL data missing or not a string in 'hero_image_url' column. Setting placeholder.");
        heroImgEl.src = placeholderImage(1920, 1080, 'Hero Image Data Missing');
        heroImgEl.alt = 'Hero image not available';
      }
    } else {
      console.error("!!! Element with ID 'hero-image' NOT FOUND in HTML !!!");
    }
    // --- AKHIR MODIFIKASI HERO IMAGE ---

    // Deskripsi About
    const aboutDescEl = document.getElementById('about-description');
    if (aboutDescEl) aboutDescEl.innerHTML = hotelProfile.description_long || 'Loading description...';

    // Key Highlights (Perbaiki HTML generation + Ikon)
    const highlightsContainer = document.getElementById('key-highlights-list');
    if (highlightsContainer && hotelProfile.key_highlights && hotelProfile.key_highlights.length > 0) {
        ...
        .map(
          (item) => `
            <div class="highlight-item flex items-start space-x-3 bg-gray-700 p-4 rounded-lg shadow"> <!-- Sesuaikan class -->
                <span class="material-icons-outlined text-cyan-400 mt-1">${item.icon || 'star'}</span>
                 <div>
                    <h4 class="font-semibold text-white">${item.title || 'Highlight'}</h4>
      ...
      highlightsContainer.innerHTML = '<p class="text-gray-500 italic col-span-full">No highlights available.</p>';
    }

    // Footer Address
    const footerAddressEl = document.getElementById('footer-address');
    if (footerAddressEl) footerAddressEl.textContent = `${hotelProfile.address?.street || ''}, ${hotelProfile.address?.city || ''}, ${hotelProfile.address?.zip_code || ''}`;
    // Footer Contact
    const footerContactEl = document.getElementById('footer-contact');
    if (footerContactEl)
      footerContactEl.innerHTML = `
        <p><a href="mailto:${hotelProfile.contact?.email_general || '#'}" class="hover:text-cyan-400">${hotelProfile.contact?.email_general || '-'}</a></p>
        <p><a href="tel:${hotelProfile.contact?.phone || '#'}" class="hover:text-cyan-400">${hotelProfile.contact?.phone || '-'}</a></p>`;
    ...
    if (footerSocialEl && hotelProfile.social_links) {
      footerSocialEl.innerHTML = `
            ...
            <div class="flex space-x-4">
              <a href="${cleanImageUrl(hotelProfile.social_links.facebook) || '#'}" target="_blank" rel="noopener noreferrer" class="hover:text-cyan-400"><i class="ri-facebook-fill text-xl"></i></a>
              <a href="${cleanImageUrl(hotelProfile.social_links.instagram) || '#'}" target="_blank" rel="noopener noreferrer" class="hover:text-cyan-400"><i class="ri-instagram-line text-xl"></i></a>
              <a href="${cleanImageUrl(hotelProfile.social_links.twitter) || '#'}" target="_blank" rel="noopener noreferrer" class="hover:text-cyan-400"><i class="ri-twitter-fill text-xl"></i></a>
            </div>
          `;
    }
    // Footer Logo (Nama Hotel)
    const footerLogoEl = document.getElementById('footer-logo');
    if (footerLogoEl) footerLogoEl.textContent = hotelProfile.name || 'Hotelux';
  } else {
  ...
  if (roomsContainer && roomTypes && roomTypes.length > 0) {
    roomsContainer.innerHTML = roomTypes
      .map((room) => {
        const imageUrl = room.gallery_images && room.gallery_images.length > 0 ? cleanImageUrl(room.gallery_images[0]) : '';
        const price = room.rate_plans && room.rate_plans.length > 0 ? room.rate_plans[0].price_per_night : 0;
        return `
            <div class="room-card bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col"> <!-- Pastikan flex flex-col -->
                <img src="${imageUrl || placeholderImage(400, 224, room.name || 'Room')}"
                     alt="${room.name || 'Room image'}"
                     class="w-full h-56 object-cover"
                     onerror="this.onerror=null; this.src='${placeholderImage(400, 224, 'Image Error')}';"> <!-- Fallback jika load error -->
                <div class="p-6 flex flex-col flex-grow"> <!-- flex-grow penting -->
                    <h3 class="text-xl font-semibold text-white mb-2">${room.name || 'Room Name'}</h3>
                    <p class="text-gray-400 leading-relaxed mb-4 flex-grow line-clamp-3">${room.description || 'No description available.'}</p> <!-- flex-grow penting -->
                    <div class="flex justify-between items-center mb-4 text-sm text-gray-400">
                      <span><i class="ri-user-line mr-1 align-middle"></i> Max ${room.occupancy?.max_adults || '?'} Adults</span>
                      <span><i class="ri-ruler-line mr-1 align-middle"></i> ${room.size_m2 || '?'} m²</span>
                    </div>
                    <div class="text-right mt-auto"> <!-- mt-auto penting -->
                      <span class="text-sm text-gray-500">Starts from</span>
                      <p class="text-xl font-bold text-cyan-400">${formatCurrency(price)}/night</p>
                    </div>
                </div>
            </div>
          `;
    ...
    roomsContainer.innerHTML = '<p class="text-gray-500 italic col-span-full text-center">No rooms available.</p>';
  }
  ...
  if (facilitiesContainer && facilities && facilities.length > 0) {
      ...
      .map(
        (category) => `
          <div class="facility-category mb-8"> <!-- Jarak antar kategori -->
              <h3 class="text-2xl font-semibold text-white mb-4">${category.category || 'Category'}</h3>
                  ...
                  ${
                    category.items && Array.isArray(category.items)
                      ...
                      <div class="flex items-center space-x-2 text-gray-300 px-2">
                          <span class="material-icons-outlined text-cyan-400 text-lg">${item.icon || 'star'}</span>
    ...
    facilitiesContainer.innerHTML = '<p class="text-gray-500 italic text-center">No facilities information available.</p>';
  }
  ...
  if (promotionsContainer && promotions && promotions.length > 0) {
      ...
      .map((promo) => {
        ...
        return `
            <div class="promo-card bg-gray-800 rounded-lg shadow-lg overflow-hidden md:flex"> <!-- flex on medium screens -->
                <img src="${imageUrl || placeholderImage(300, 200, promo.title || 'Promo')}"
                     alt="${promo.title || 'Promotion image'}"
                     class="w-full md:w-1/3 h-48 md:h-auto object-cover"
                     onerror="this.onerror=null; this.src='${placeholderImage(300, 200, 'Image Error')}';">
                <div class="p-6 flex flex-col justify-center md:w-2/3">
                  <h3 class="text-xl font-semibold text-white mb-2">${promo.title || 'Special Offer'}</h3>
                  <p class="text-gray-400 mb-4 text-sm leading-relaxed">${promo.description || ''}</p>
                  <span class="text-sm font-medium text-cyan-400">Use Code: <strong class="bg-gray-700 px-2 py-1 rounded text-cyan-300">${promo.promo_code || 'N/A'}</strong></span>
                </div>
            </div>
           `;
      })
    ...
    promotionsContainer.innerHTML = '<p class="text-gray-500 italic col-span-full text-center">No current promotions.</p>';
  }
  ...
  if (testimonialsContainer && testimonials && testimonials.length > 0) {
         ...
         <div class="testimonial-card bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col"> <!-- flex -->
             <div class="flex items-center mb-4">
               ...
               <div class="ml-3">
                 ...
                 <p class="text-sm text-gray-500">${t.origin || 'Unknown Location'}</p>
               </div>
             </div>
             <p class="text-gray-400 italic mb-4 flex-grow">"${t.quote || 'No comment provided.'}"</p> <!-- flex-grow -->
             <div class="flex mt-auto"> <!-- Rating stars -->
               ${[...Array(5)]
                 .map(
                   (_, i) => `
                 <span class="material-icons-outlined text-sm ${i < (t.rating || 0) ? 'text-yellow-400' : 'text-gray-600'}">
    ...
    testimonialsContainer.innerHTML = '<p class="text-gray-500 italic col-span-full text-center">No testimonials yet.</p>';
  }
  ...
  if (bookingSelect && roomTypes && roomTypes.length > 0) {
    ...
    bookingSelect.innerHTML = '<option value="">-- Select a Room Type --</option>'; // Placeholder lebih baik
    roomTypes.forEach((room) => {
      if (room.rate_plans && room.rate_plans.length > 0) {
        const option = document.createElement('option');
        option.value = room.id;
        const price = room.rate_plans[0].price_per_night;
        option.textContent = `${room.name || 'Unnamed Room'} (${formatCurrency(price)}/night)`;
    ...
    bookingSelect.innerHTML = '<option value="">-- No rooms available to book --</option>';
  }
...
if (roomSelectElement) roomSelectElement.addEventListener('change', calculateBookingPrice);
if (checkInElement) checkInElement.addEventListener('change', calculateBookingPrice);
if (checkOutElement) checkOutElement.addEventListener('change', calculateBookingPrice);
...
if (bookingForm) {
  bookingForm.addEventListener('submit', async (e) => {
    ...
    if (!bookingData.guest_email || !/\S+@\S+\.\S+/.test(bookingData.guest_email)) errors.push('Valid Email is required.');
    ...
    if (bookingData.check_in && bookingData.check_out && new Date(bookingData.check_out) <= new Date(bookingData.check_in)) errors.push('Check-out date must be after check-in date.');
    if (!bookingData.total_price || bookingData.total_price <= 0) errors.push('Total price seems invalid. Please re-select dates or room.');
    ...
    if (errors.length > 0) {
      ...
      showNotification(
        'error', 
        'Booking Incomplete', 
        `<ul class="list-disc pl-4 text-left space-y-1 text-sm">${errors.map((err) => `<li>${err}</li>`).join('')}</ul>`
      );
    ...
    if (error) {
      ...
      showNotification('error', 'Something Went Wrong', `We couldn't process your booking. <br><span class="text-xs opacity-70">Error: ${error.message}</span>`);
    } else {
